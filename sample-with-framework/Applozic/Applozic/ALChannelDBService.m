...
//        DB_CHANNEL *dbChannel = [self createChannelEntity:channel];
    ...
    if(error)
    {
        ALSLogBasic(ALLoggerSeverityError, @"ERROR IN insertChannel METHOD %@",error);
    }
}
...
//        DB_CHANNEL_USER_X *dbChannelUserX = [self createChannelUserXEntity:channelUserX];
    ...
    if(error)
    {
        ALSLogBasic(ALLoggerSeverityError, @"ERROR IN insertChannelUserX METHOD %@",error);
    }
...
-(NSMutableArray *)getChannelMembersList:(NSNumber *)channelKey
{
    ...
    if (error)
    {
        ALSLogBasic(ALLoggerSeverityError, @"ERROR IN FETCH MEMBER LIST");
    }
    else
    {
...
-(void)removeMemberFromChannel:(NSString *)userId andChannelKey:(NSNumber *)channelKey
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityWarn, @"NO MEMBER FOUND");
    }
}
...
-(void)deleteChannel:(NSNumber *)channelKey
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityWarn, @"NO ENTRY FOUND");
    }
}
...
-(void)updateChannel:(NSNumber *)channelKey andNewName:(NSString *)newName orImageURL:(NSString *)imageURL orChildKeys:(NSMutableArray *)childKeysList isUpdatingMetaData:(BOOL)flag  orChannelUsers:(NSMutableArray *)channelUsers{
    ...
    {
        ALSLogBasic(ALLoggerSeverityError, @"UPDATE_CHANNEL_DB : NO CHANNEL FOUND");
    }
}
...
-(void)updateUnreadCountChannel:(NSNumber *)channelKey unreadCount:(NSNumber *)unreadCount
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityError, @"NO CHANNEL FOUND");
    }
}
...
-(void)setLeaveFlag:(BOOL)flag forChannel:(NSNumber *)groupId
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityError, @"NO CHANNEL : %@ FOUND",groupId);
    }
}
...
-(NSUInteger)markConversationAsRead:(NSNumber*)channelKey
{
    ...
    if(channelKey){
        ...
        ALSLogBasic(ALLoggerSeverityError, @"channelKey null for marking unread");
    }
    ...
    if(messages.count > 0)
    {
        ...
        ALSLogBasic(ALLoggerSeverityInfo, @"%@ objects updated", res.result);
    }
...
-(DB_CHANNEL *)getChannelByClientChannelKey:(NSString *)clientChannelKey
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityError, @"CHANNEL_NOT_FOUND :: %@",clientChannelKey);
        return nil;
    }
}
...
-(NSMutableArray *)fetchChildChannels:(NSNumber *)parentGroupKey
{
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"CHILD CHANNEL FOUND : %lu WITH PARENT KEY : %@",result.count, parentGroupKey);
    ALSLogBasic(ALLoggerSeverityError, @"ERROR (IF-ANY) : %@",fetchError.description);
