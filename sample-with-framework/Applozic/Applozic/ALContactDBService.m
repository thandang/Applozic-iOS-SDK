...
- (BOOL)purgeListOfContacts:(NSArray *)contacts {
    ...
    for (ALContact *contact in contacts) {
        ...
        if (!result) {
            ...
            ALSLogBasic(ALLoggerSeverityInfo, @"Failure to delete the contacts");
            break;
        }
    }
...
- (BOOL)updateListOfContacts:(NSArray *)contacts {
    ...
    for (ALContact *contact in contacts) {
        ...
        if (!result) {
            ...
            ALSLogBasic(ALLoggerSeverityInfo, @"Failure to update the contacts");
            break;
        }
    }
...
- (BOOL)updateContact:(ALContact *)contact {
    ...
    if (!success) {
        ...
        ALSLogBasic(ALLoggerSeverityError, @"updateContactFERROR :%@",error);
    }
...
-(NSUInteger)markConversationAsDeliveredAndRead:(NSString*)contactId
{
    ...
    ALSLogBasic(ALLoggerSeverityError, @"ERROR(IF-ANY) WHILE UPDATING DELIVERED_AND_READ : %@",error.description);
...
-(BOOL)setBlockUser:(NSString *)userId andBlockedState:(BOOL)flag
{
    ...
    if (!success)
    {
        ALSLogBasic(ALLoggerSeverityError, @"DB ERROR FOR BLOCKING/UNBLOCKING USER %@ :%@",userId, error);
    }
    return success;
}
...
-(BOOL)setBlockByUser:(NSString *)userId andBlockedByState:(BOOL)flag
{
    ...
    if (!success)
    {
        ALSLogBasic(ALLoggerSeverityError, @"DB ERROR FOR BLOCKED BY USER %@ :%@", userId, error);
    }
    return success;
}
...
-(NSMutableArray *)getListOfBlockedUsers
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"NO BLOCKED USER FOUND");
    }
...
-(BOOL)insertNewContact:(ALContact*)contact inContext:(NSManagedObjectContext*)context
{
    ...
    if (!result) {
        ALSLogBasic(ALLoggerSeverityError, @"addContact DB ERROR :%@",error);
    }
    return result;
}
