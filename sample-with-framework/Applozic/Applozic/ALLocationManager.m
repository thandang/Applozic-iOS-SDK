...
-(void)getAddress {
    ...
    [self setup];
    ALSLogBasic(ALLoggerSeverityInfo, @"get Address...");
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"didUpdateToLocation: %@", newLocation);
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations
{
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"didUpdateLocations called:");
...
-(void) handleLocationUpdate:(CLLocation * )currentLocation {
    ...
    // Reverse Geocoding
    ALSLogBasic(ALLoggerSeverityInfo, @"Resolving the Address");
    _geocoder = [[CLGeocoder alloc]init];
    
    [self.geocoder reverseGeocodeLocation:currentLocation completionHandler:^(NSArray *placemarks, NSError *error) {
        NSMutableDictionary * dict =  [[NSMutableDictionary alloc]init];
        if(error){
            ALSLogBasic(ALLoggerSeverityError, @"%@", [error localizedDescription]);
