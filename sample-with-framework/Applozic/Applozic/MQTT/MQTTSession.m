...
- (instancetype)init {
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] init");
    self = [super init];
    self.txMsgId = 1;
...
- (UInt16)subscribeToTopics:(NSDictionary<NSString *, NSNumber *> *)topics subscribeHandler:(MQTTSubscribeHandler)subscribeHandler {
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] subscribeToTopics:%@]", topics);
...
- (UInt16)unsubscribeTopics:(NSArray<NSString *> *)topics unsubscribeHandler:(MQTTUnsubscribeHandler)unsubscribeHandler {
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] unsubscribeTopics:%@", topics);
    UInt16 mid = [self nextMsgId];
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] publishData:%@... onTopic:%@ retain:%d qos:%ld publishHandler:%p",
    ...
    if (qos) {
        ...
        if (!flow) {
            ALSLogBasic(ALLoggerSeverityWarn, @"[MQTTSession] dropping outgoing message %d", msgId);
            ...
            if ([flow.commandType intValue] == MQTTPublish) {
                ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] PUBLISH %d", msgId);
                if (![self encode:msg]) {
                    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] queueing message %d after unsuccessfull attempt", msgId);
                    flow.commandType = [NSNumber numberWithUnsignedInt:MQTT_None];
                    flow.deadline = [NSDate date];
                    [self.persistence sync];
                }
            } else {
                ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] queueing message %d", msgId);
            }
        }
    } else {
        NSError *error = nil;
...
- (void)closeWithDisconnectHandler:(MQTTDisconnectHandler)disconnectHandler {
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] closeWithDisconnectHandler:%p ", disconnectHandler);
    self.disconnectHandler = disconnectHandler;
    
    if (self.status == MQTTSessionStatusConnected) {
        ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] disconnecting");
...
- (void)closeInternal
{
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] closeInternal");
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] keepAlive %@ @%.0f", self.clientId, [[NSDate date] timeIntervalSince1970]);
    (void)[self encode:[MQTTMessage pingreqMessage]];
}

- (void)checkDup:(NSTimer *)timer
{
    ///ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] checkDup %@ @%.0f", self.clientId, [[NSDate date] timeIntervalSince1970]);
    [self checkTxFlows];
}
...
- (void)checkTxFlows {
    ...
    for (id<MQTTFlow> flow in flows) {
        ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] %@ flow %@ %@ %@", self.clientId, flow.deadline, flow.commandType, flow.messageId);
            ...
            switch ([flow.commandType intValue]) {
                case 0:
                    if (windowSize <= self.persistence.maxWindowSize) {
                        ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] PUBLISH queued message %@", flow.messageId);
                    ...
                    break;
                case MQTTPublish:
                    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] resend PUBLISH %@", flow.messageId);
                    message = [MQTTMessage publishMessageWithData:flow.data
                                                          onTopic:flow.topic
                                                              qos:[flow.qosLevel intValue]
                                                            msgId:[flow.messageId intValue]
                    ...
                    break;
                case MQTTPubrel:
                    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] resend PUBREL %@", flow.messageId);
...
- (void)decoder:(MQTTDecoder*)sender handleEvent:(MQTTDecoderEvent)eventCode error:(NSError *)error {
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] decoder handleEvent: %@ (%d) %@",
...
- (void)decoder:(MQTTDecoder*)sender didReceiveMessage:(NSData *)data {
    ...
    if (!message) {
        ALDDLogError(@"[MQTTSession] MQTT illegal message received");
        NSError * error = [NSError errorWithDomain:MQTTSessionErrorDomain
    ...
    @synchronized(sender) {
        ...
        switch (self.status) {
            ...
            case MQTTSessionStatusConnected:
                ...
                break;
            default:
                ALDDLogError(@"[MQTTSession] other state");
                break;
        }
    }
}
...
- (void)handlePublish:(MQTTMessage*)msg {
    ...
    if (!topic) {
        ...
        ALDDLogError(@"non UTF8 topic %@", topic);
    }
                ...
                if (![self.persistence storeMessageForClientId:self.clientId
                                                  ...
                                                  incomingFlag:YES
                                                   commandType:MQTTPubrec
                                                      deadline:[NSDate dateWithTimeIntervalSinceNow:DUPTIMEOUT]]) {
                    ALSLogBasic(ALLoggerSeverityWarn, @"[MQTTSession] dropping incoming messages");
                } else {
                    [self.persistence sync];
                    [self tell];
...
- (UInt16)nextMsgId {
    ALSLogBasic(ALLoggerSeverityInfo, @"nextMsgId synchronizing");
    @synchronized(self) {
        ALSLogBasic(ALLoggerSeverityInfo, @"nextMsgId synchronized");
        self.txMsgId++;
        ...
        ALSLogBasic(ALLoggerSeverityInfo, @"nextMsgId synchronized done");
        return self.txMsgId;
    }
}
...
- (void)connect {
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] connecting");
...
- (void)connectWithConnectHandler:(MQTTConnectHandler)connectHandler {
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] connectWithConnectHandler:%p", connectHandler);
    self.connectHandler = connectHandler;
    [self connect];
}

- (void)disconnect {
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] sending DISCONNECT");
    self.status = MQTTSessionStatusDisconnecting;
...
- (BOOL)encode:(MQTTMessage *)message {
    if (message) {
        ...
        if (wireFormat) {
            ...
            ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] mqttTransport send");
            return [self.transport send:wireFormat];
        } else {
            ALDDLogError(@"[MQTTSession] trying to send message without wire format");
            return false;
        }
    } else {
        ALDDLogError(@"[MQTTSession] trying to send nil message");
        return false;
    }
}
...
- (void)mqttTransport:(id<MQTTTransport>)mqttTransport didReceiveMessage:(NSData *)message {
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] mqttTransport didReceiveMessage");
...
- (void)mqttTransportDidClose:(id<MQTTTransport>)mqttTransport {
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] mqttTransport mqttTransportDidClose");
...
- (void)mqttTransportDidOpen:(id<MQTTTransport>)mqttTransport {
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] mqttTransportDidOpen");
    
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTSession] sending CONNECT");
...
- (void)mqttTransport:(id<MQTTTransport>)mqttTransport didFailWithError:(NSError *)error {
    ALSLogBasic(ALLoggerSeverityWarn, @"[MQTTSession] mqttTransport didFailWithError %@", error);
