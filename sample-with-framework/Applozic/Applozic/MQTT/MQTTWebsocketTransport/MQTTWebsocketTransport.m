...
- (void)open {
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTWebsocketTransport] open");
...
- (BOOL)send:(nonnull NSData *)data {
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTWebsocketTransport] send(%ld):%@", (unsigned long)data.length,
...
- (void)close {
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTWebsocketTransport] close");
...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message {
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTWebsocketTransport] didReceiveMessage(%ld)", (unsigned long)(data ? data.length : -1));
...
- (void)webSocketDidOpen:(SRWebSocket *)webSocket{
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTWebsocketTransport] connected to websocket");
...
- (void)webSocket:(SRWebSocket *)webSocket didFailWithError:(NSError *)error{
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTWebsocketTransport] Failed to connect : %@",[error debugDescription]);
...
- (void)webSocket:(SRWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean{
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTWebsocketTransport] ConnectionClosed : %@",reason);
    self.state = MQTTTransportClosed;
    self.websocket.delegate = nil;
    if (self.delegate) {
...
- (void)webSocket:(SRWebSocket *)webSocket didReceivePong:(NSData *)pongPayload{
    ALSLogBasic(ALLoggerSeverityInfo, @"[MQTTWebsocketTransport] webSocket didReceivePong:%@", pongPayload);
}
