...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"INDIVIDUAL_LAUNCH :: SUBSCRIBING_MQTT");
        ...
        {
            ALSLogBasic(ALLoggerSeverityInfo, @"mqttObject is not found...");
        }
    }
...
-(void)messageDeletedAPPLOZIC05Handler:(NSNotification *)notification{
    ...
    if(proccessfilterArray.count != 0)
    {
        ...
        ALSLogBasic(ALLoggerSeverityInfo, @"Messsage 05:%@",msg.message);
        msg.deleted = YES;
        ...
        [ALMessageService deleteMessage:messageKey andContactId:self.contactIds withCompletion:^(NSString * response, NSError * error) {
            ...
            ALSLogBasic(ALLoggerSeverityInfo, @"Message Deleted upon APPLOZIC_05",response);
...
-(void)viewWillDisappear:(BOOL)animated
{
    ...
    if(self.individualLaunch)
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"ALChatVC: Individual launch ...unsubscribeToConversation to mqtt..");
        if(self.mqttObject)
        {
            [self.mqttObject unsubscribeToConversation];
            ALSLogBasic(ALLoggerSeverityInfo, @"ALChatVC: In ViewWillDisapper .. MQTTObject in ==IF== now");
        }
        else
        {
            ALSLogBasic(ALLoggerSeverityInfo, @"mqttObject is not found...");
...
-(void)processResettingUnreadCount
{
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"CHATVC_UNREAD_COUNT :: %i",count);
    if(count == 0)
    {
        [userService resettingUnreadCountWithCompletion:^(NSString *json, NSError *error) {
            
            ALSLogBasic(ALLoggerSeverityError, @"RESET_UNREAD_COUNT CALL :: %@ and ERROR :: %@",json, error.description);
        }];
    }
}
...
-(void)markConversationRead
{
    ...
    if(self.channelKey && isGroupNotification)
    {
        [ALChannelService markConversationAsRead:self.channelKey withCompletion:^(NSString * string, NSError * error) {
            ...
            if(error)
            {
                ALSLogBasic(ALLoggerSeverityError, @"Error while marking messages as read channel %@",self.channelKey);
            }
            else
            {
    ...
    if(self.contactIds && !self.isGroup)
    {
        [ALUserService markConversationAsRead:self.contactIds withCompletion:^(NSString * string, NSError *error) {
            if(error)
            {
                ALSLogBasic(ALLoggerSeverityError, @"Error while marking messages as read for contact %@", self.contactIds);
            }
            else
            {
                [self processResettingUnreadCount];
    ...
    if(almessage.groupId != NULL)
    {
        if([self.channelKey isEqualToNumber:almessage.groupId])
        {
            [ALUserService markMessageAsRead:almessage withPairedkeyValue:almessage.pairedMessageKey withCompletion:^(NSString * completion, NSError * error) {
                ...
                if(error)
                {
                    ALSLogBasic(ALLoggerSeverityError, @"GROUP: Marking message read error:%@",error);
                }
            }];
        }
    }
    else
    {
        if([self.contactIds isEqualToString:almessage.contactIds])
        {
            [ALUserService markMessageAsRead:almessage withPairedkeyValue:almessage.pairedMessageKey withCompletion:^(NSString * completion, NSError * error) {
                ...
                if(error)
                {
                    ALSLogBasic(ALLoggerSeverityError, @"Individual: Marking message read error:%@",error);
                }
            }];
        }
    }
...
-(void)checkUserBlockStatus
{
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"USER_STATE BLOCKED : %i AND BLOCKED BY : %i", contact.block, contact.blockBy);
    ALSLogBasic(ALLoggerSeverityInfo, @"USER : %@", contact.userId);
...
-(void)setupPickerView
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"No conversation list ");
        return;
    }
    ...
    for(ALConversationProxy * conversation in conversationList)
    {
        ...
        {
            ALSLogBasic(ALLoggerSeverityError, @"<< ERROR: Topic Detail NILL >>");
        }
    }
...
-(void)setButtonTitle
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"CURENT clientChannelKey :: %@",self.alChannel.clientChannelKey);
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"fetchMessageFromDB  called:: ");
    ALDBHandler * theDbHandler = [ALDBHandler sharedInstance];
...
-(void)sendLocationMessage:(ALMessage *)theMessage withCompletion:(void(^)(NSString *message, NSError *error))completion
{
    [ALMessageService sendMessages:theMessage withCompletion:^(NSString *message, NSError *error) {
        ...
        if(error)
        {
            ALSLogBasic(ALLoggerSeverityError, @"SEND_MSG_ERROR :: %@", error.description);
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"Google Map Length = ZERO");
...
-(void)showConversationPicker:(UITapGestureRecognizer *)recognizer
{
    ...
    if(NSNotFound == anIndex) {
        ALSLogBasic(ALLoggerSeverityInfo, @"PickerView Index not found %ld",(long)anIndex);
        return;
    }
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"  deleteMessageFromView in controller...:: ");
...
-(void)downloadRetryButtonActionDelegate:(int)index andMessage:(ALMessage *)message
{
    ...
    if (theFiletredArray.count == 0)
    {
        ...
        ALSLogBasic(ALLoggerSeverityInfo, @"starting thread for..%@", message.key);
    }
    else
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"connection already present do nothing###");
    }
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"Called get image stopDownloadForIndex stopDownloadForIndex ####");
...
-(void)connection:(ALConnection *)connection didReceiveData:(NSData *)data
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @" file posting done");
        return;
    }
...
-(void)connection:(ALConnection *)connection didFailWithError:(NSError *)error
{
    ...
    ALSLogBasic(ALLoggerSeverityError, @"didFailWithError ::: %@",error);
    [[[ALConnectionQueueHandler sharedConnectionQueueHandler] getCurrentConnectionQueue] removeObject:connection];
}
...
-(void)releaseConnection:(NSString *)key
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"CONNECTION_NOT_FOUND");
    }
}
...
//        [self.sendMessageTextView setText:nil];
        ...
        if(!imageCell)
        {
            ALSLogBasic(ALLoggerSeverityInfo, @" not able to find the image cell for upload....");
            return;
        }
...
-(void)fetchAndRefresh:(BOOL)flag
{
    ...
    [ALMessageService getLatestMessageForUser:deviceKeyString withCompletion:^(NSMutableArray  *messageList, NSError *error) {
        ...
        if(error)
        {
            ALSLogBasic(ALLoggerSeverityError, @"ERROR_GetLatestMessageForUser :: %@",error);
            return ;
        }
        else
        {
            ...
            ALSLogBasic(ALLoggerSeverityInfo, @"FETCH AND REFRESH METHOD");
        }
    }];
}
...
-(void)updateStatusReportForConversation:(int)status
{
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"Found Messages to update to DELIVERED_AND_READ in ChatView :%lu", (unsigned long)filteredArray.count);
...
-(void)individualNotificationhandler:(NSNotification *) notification
{
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @" OUR Individual Notificationhandler ");
    [self setRefreshMainView:TRUE];
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"Notification received by Individual chat list: %@", contactId);
...
-(void)syncCall:(ALMessage*)alMessage  updateUI:(NSNumber *)updateUI alertValue: (NSString *)alertValue
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"it was in background, updateUI is false");
        self.conversationId = alMessage.conversationId;
        self.channelKey = alMessage.groupId;
        self.contactIds = alMessage.contactIds;
       // [self fetchAndRefresh:YES];
    ...
    else
    {
        ...
        {
            ALSLogBasic(ALLoggerSeverityInfo, @"SHOW_NOTIFICATION (OTHER_THREAD_IS_OPENED)");
...
-(void)processLoadEarlierMessages:(BOOL)isScrollToBottom
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @" time### %@ ",time);
        time = NULL;
    }
    ...
    [ALMessageService getMessageListForUser:messageListRequest  withCompletion:^(NSMutableArray *messages, NSError *error, NSMutableArray *userDetailArray) {
        ...
        [self.mActivityIndicator stopAnimating];
        ALSLogBasic(ALLoggerSeverityInfo, @"LIST_CALL_CALLED");
            ...
            for (ALMessage * msg in messages)
            {
                if([self.alMessageWrapper getUpdatedMessageArray].count > 0)
                {
                    ...
                    if(msg1.createdAtTime.doubleValue <= msg.createdAtTime.doubleValue) {
                        ALSLogBasic(ALLoggerSeverityInfo, @"ignoring as coming message has grater time..continue....");
                        continue;
                    }
                ...
                {
                    //ALSLogBasic(ALLoggerSeverityInfo, @"insterting message at index 0 ::%@", msg.key);
        ...
        {
            self.loadEarlierAction.hidden = YES;
            ALSLogBasic(ALLoggerSeverityError, @"some error");
        }
    }];
}
...
-(void)serverCallForLastSeen
{
    ...
    [ALUserService userDetailServerCall:self.contactIds withCompletion:^(ALUserDetail *alUserDetail)
    {
        ...
        {
            ALSLogBasic(ALLoggerSeverityInfo, @"CHECK LAST_SEEN_SERVER CALL");
        }
    }];
}
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"USER DET : %@",alUserDetail.userId);
    ALSLogBasic(ALLoggerSeverityInfo, @"self.contactIds : %@",self.contactIds);
    [self setRefreshMainView:TRUE];
...
-(NSMutableArray *)getLastSeenForGroupDetails
{
    ...
    for (NSString * userID in memberIdArray)
    {
        ...
        ALSLogBasic(ALLoggerSeverityInfo, @"Contact :: %@ && Value :: %@", contact.userId, contact.lastSeenAt);
...
-(void)mqttDidConnected {
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"MQTT_CONNECTED : CALL BACK COMES ALCHATVC");
    if (self.individualLaunch) {
        [self subscrbingChannel];
    }
}
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"ALCHATVC : USER_DETAIL_CHANGED_CALL_UPDATE");
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"ALCHATVC : USER_DETAIL_SUB_PROCESS");
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"==== (CHAT_VC) Received typing status %d for: %@ ====", status, userId);
...
-(void)mqttConnectionClosed
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"MQTT connection closed, subscribing again: %lu", (long)_mqttRetryCount);
        [self.mqttObject subscribeToConversation];
        [self subscrbingChannel];
        self.mqttRetryCount++;
    }
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"will enter foreground notification");
...
-(void)addMessageToList:(NSMutableArray  *)messageList
{
    ...
    if(sortedArray.count==0){
        ALSLogBasic(ALLoggerSeverityInfo, @"No message for contact .....%@",self.contactIds);
        return;
    }
    [self setTitle];
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @" newMessageHandler called ::#### ");
...
-(void) scrollToReplyMessage:(ALMessage *)alMessage
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"Reply cell not found..");
    }
