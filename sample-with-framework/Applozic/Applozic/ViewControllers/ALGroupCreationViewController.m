...
- (void)updateGroupInfo:(id)sender
{
    ...
    [channelService updateChannel:self.channelKey andNewName:changedName
          ...
          if(!error)
          {
              ...
              ALSLogBasic(ALLoggerSeverityInfo, @"ALGroupCreationViewController updated the group info");
...
{
    ALSLogBasic(ALLoggerSeverityError, @"GROUP_IMAGE UPLOAD_ERROR :: %@",error.description);
    [self.navigationItem.rightBarButtonItem setEnabled:YES];
    [self.activityIndicator stopAnimating];
}
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"GROUP_IMAGE UPLOAD PROGRESS :: %lu out of %lu",totalBytesWritten,totalBytesExpectedToWrite);
}
...
-(void)connectionDidFinishLoading:(ALConnection *)connection
{
    ...
    if([connection.connectionType isEqualToString:CONNECTION_TYPE_GROUP_IMG_UPLOAD])
    {
        ...
        ALSLogBasic(ALLoggerSeverityInfo, @"GROUP_IMAGE_LINK :: %@",imageLinkFromServer);
