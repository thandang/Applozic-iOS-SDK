...
-(void)viewDidLoad
{
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"## self.alChannel ::", self.alChannel.notificationAfterTime);
}
...
-(void)handleAPNS:(NSNotification *)notification
{
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"GROUP_DETAIL_VC_NOTIFICATION_OBJECT : %@",contactId);
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"######## GROUP DETAIL VC : APP_STATE_INACTIVE #########");
...
-(void)getDisplayNamesAndLastSeen
{
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"Member Count :%ld",(long)self.memberCount);
}
    ...
    if(buttonIndex == 1)
    {
        ...
        else
        {
            ...
            [ALMessageService deleteMessageThread:nil orChannelKey:self.channelKeyID withCompletion:^(NSString *string, NSError *error) {
                ...
                if(error)
                {
                    ALSLogBasic(ALLoggerSeverityError, @"DELETE FAILED: Unable to delete contact conversation : %@", error.description);
...
-(void)updateGroupView
{
    ...
    if([ALApplozicSettings isGroupInfoEditDisabled] || [ALChannelService isConversationClosed: alchannel.key] ){
        ALSLogBasic(ALLoggerSeverityInfo, @"group edit is disabled");
        return;
    }
...
-(void) sendMuteRequestWithTime:(NSNumber*) time{
    ...
    [alChannelService muteChannel:alMuteRequest withCompletion:^(ALAPIResponse *response, NSError *error) {
        ALSLogBasic(ALLoggerSeverityInfo, @"actionSheet response from server:: %@", response.status);
        [[self activityIndicator] stopAnimating];
