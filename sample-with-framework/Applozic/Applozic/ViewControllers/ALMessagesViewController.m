...
-(void)viewDidDisappear:(BOOL)animated
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"MSG VC : CLOSING_MQTT_CONNECTIONS");
...
-(void)refreshMessageList
{
    ...
    [ALMessageService getLatestMessageForUser:[ALUserDefaultsHandler getDeviceKeyString] withCompletion:^(NSMutableArray  * messageList, NSError *error) {
        ...
        if(error)
        {
            ALSLogBasic(ALLoggerSeverityError, @"ERROR: IN REFRESH MSG VC :: %@",error);
            return;
        }
        ALSLogBasic(ALLoggerSeverityInfo, @"REFRESH MSG VC");
    }];
}
...
-(void)getMessagesArray:(NSMutableArray *)messagesArray
{
    ...
    [self.mTableView reloadData];
    ALSLogBasic(ALLoggerSeverityInfo, @"GETTING MESSAGE ARRAY");
}
...
//            [voipHandler handleAVMsg:msg andViewController:self];
        ...
        else
        {
           index = [self.mContactsMessageListArray indexOfObjectPassingTest:^BOOL(ALMessage *almessage, NSUInteger idx, BOOL *stop) {
            ...
            ALSLogBasic(ALLoggerSeverityInfo, @"contact cell not found ....");
        }
    }
    if(isreloadRequire)
    {
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete)
    {
        ...
        ALSLogBasic(ALLoggerSeverityInfo, @"DELETE_PRESSED");
        if(![ALDataNetworkConnection checkDataNetworkAvailable])
        {
            [self noDataNotificationView];
            return;
        ...
        [ALMessageService deleteMessageThread:alMessageobj.contactIds orChannelKey:[alMessageobj getGroupId]
            ...
            if(error)
            {
                ALSLogBasic(ALLoggerSeverityError, @"DELETE_FAILED_CONVERSATION_ERROR_DESCRIPTION :: %@", error.description);
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"GETTING_FILTERED_ARRAY_COUNT :: %lu", (unsigned long)theFilteredArray.count);
...
-(void)updateConversationTableNotification:(NSNotification *)notification
{
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"NOTIFICATION_FOR_TABLE_UPDATE :: %@", theMessage.message);
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"ALMSGVC : USER_DETAIL_CHANGED_CALL_UPDATE");
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"==== (MSG_VC) Received typing status %d for: %@ ====", status, userId);
    ALContactDBService *contactDBService = [[ALContactDBService alloc] init];
...
-(void)mqttConnectionClosed
{
    ...
    if([ALDataNetworkConnection checkDataNetworkAvailable] && !isBackgroundState)
    {
        ...
        ALSLogBasic(ALLoggerSeverityInfo, @"ALMessageVC subscribing channel again....");
        [self.alMqttConversationService subscribeToConversation];
        self.mqttRetryCount++;
    }
}
...
-(void)pushNotificationhandler:(NSNotification *) notification
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"######## IT SHOULD NEVER COME HERE #########");
...
-(BOOL)getVisibleState
{
    ...
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"VIEW_CONTROLLER IS VISIBLE");
        return YES;
    }
    else
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"VIEW_CONTROLLER IS NOT VISIBLE");
        return NO;
    }
}
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if(self.parentGroupKey && [ALApplozicSettings getSubGroupLaunchFlag])
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"NOT REQUIRE FOR PARENT GROUP");
        return;
    }
    
    ALSLogBasic(ALLoggerSeverityInfo, @"END_SCROCLLING_TRY");
