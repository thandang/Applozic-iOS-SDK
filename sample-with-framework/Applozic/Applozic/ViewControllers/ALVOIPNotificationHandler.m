...
+(void)sendMessageWithMetaData:(NSMutableDictionary *)dictionary
    ...
    [ALMessageService sendMessages:messageWithMetaData withCompletion:^(NSString *message, NSError *error) {
        ...
        ALSLogBasic(ALLoggerSeverityInfo, @"AUDIO/VIDEO MSG_RESPONSE :: %@",message);
        ALSLogBasic(ALLoggerSeverityError, @"ERROR IN AUDIO/VIDEO MESSAGE WITH META-DATA : %@", error);
    }];
}
...
-(void)handleAVMsg:(ALMessage *)alMessage andViewController:(UIViewController *)viewSelf
{
    ...
    if (alMessage.contentType == AV_CALL_CONTENT_TWO)
    {
        ...
        {
            ALSLogBasic(ALLoggerSeverityInfo, @" video/audio call not enables  ");
            return;
        }
        ...
        if([msgType isEqualToString:@"CALL_DIALED"])
        {
            ...
            else if (appObject.applicationState == UIApplicationStateBackground)
            {
                ...
                self.backgroundTask = [appObject beginBackgroundTaskWithExpirationHandler:^{
                    ALSLogBasic(ALLoggerSeverityInfo, @"ALVOIP : BACKGROUND_HANDLER_NO_MORE_TASK_RUNNING.");
        ...
        else if ([msgType isEqualToString:@"CALL_REJECTED"])
        {
            ...
            ALSLogBasic(ALLoggerSeverityInfo, @"CALL_IS_REJECTED");
            [self.baseAV dismissAVViewController:YES];
            [self invalidateCallNotifying];
        ...
        {
            ALSLogBasic(ALLoggerSeverityInfo, @"CALL_IS_MISSED");
            [self.baseAV dismissAVViewController:YES];
...
-(void)showIncomingCall:(NSTimer *)timer
{
    if (count < 60)
    {
        ALSLogBasic(ALLoggerSeverityInfo, @"BG_TIME_REMAIN : %f",appObject.backgroundTimeRemaining);
        if (IS_OS_EARLIER_THAN_10)
        {
        ...
        else
        {
            ...
            [center addNotificationRequest:request withCompletionHandler:^(NSError * _Nullable error) {
                if (!error) {
                    ALSLogBasic(ALLoggerSeverityInfo, @"PUSHKIT : INCOMING_VOIP_APN");
                }
            }];
...
- (void)application:(UIApplication *)application didReceiveLocalNotification:(UILocalNotification *)notification {
    ...
    ALSLogBasic(ALLoggerSeverityInfo, @"ALVOIP : DID_RECEIVE_LOCAL_NOTIFICATION");
...
{
    ALSLogBasic(ALLoggerSeverityInfo, @"[[NSDate date]timeIntervalSince1970] - [alMessage.createdAtTime doubleValue] ::%d", [[NSDate date]timeIntervalSince1970]*1000 - [alMessage.createdAtTime doubleValue]);
